...
describe("jobService", () => {
  ...
  it("skips Job 1 entirely when a sealed run already exists", async () => {
    const selectRunResult = { decision: "already_sealed" as const, run_id: "example-20260815-001" };
    const postMock = vi.fn((path: string, body: unknown): Promise<unknown> => {
      if (path === "/session/handshake") {
        return Promise.resolve({
          message: "ok",
      ...
      if (path === "/jobs/request") {
        expect((body as RequestJobRequest).localExecution).toEqual({ select_run: selectRunResult });
        return Promise.resolve(job);
      }
    ...
    const client: HttpClient = {
      get: (path, schema) => {
        if (path.startsWith("/workflows/current")) {
          return Promise.resolve(
            schema.parse({ stages: [], workflowId: "mock", workflowVersion: "1.0.0" })
          );
        }
        ...
        setTimeout(() => {
          for (const callback of outputCallbacks) {
            callback({
              chunk: {
                stream: "stdout",
                text: '{"ok":true,"decision":"already_sealed","run_id":"example-20260815-001"}\n',
                timestamp: "2026-08-14T00:00:01.000Z"
              },
              providerId: PROVIDER_IDS.claudeCode,
              taskId: "44444444-4444-4444-8444-444444444444"
    ...
    const runStartupJobMock = vi.fn(() =>
      Promise.resolve({
        check_factory: { created: true, path: "C:\\Github\\ForgePilot\\.ai-factory" },
        read_config: { locale: "tr-TR", mode: "unknown", version: "unknown" }
      })
    );
    const service = createJobService({
      createClient: () => client,
    ...
    expect(runStartupJobMock).not.toHaveBeenCalled();
  ...
  describe("020-discovery", () => {
    ...
    const buildContextPreparationResult = {
      businessTerms: [],
      documents: [],
      entities: [],
      manifestDescriptionCandidates: [],
      modules: [{ id: "root", name: "root", paths: [], root: ".", summary: { file_count: 0, formats: [], kind_counts: {}, signals: [] } }],
      projectName: "ForgePilot",
      technologyStack: [],
      userRoles: []
    };
      ...
      expect(requestedLocalExecutions).toEqual([
        ...
        { index_documents: indexDocumentsResult, map_dependencies: mapDependenciesResult },
        {
          build_context_evidence: {
            businessTerms: buildContextPreparationResult.businessTerms,
            documents: buildContextPreparationResult.documents,
            manifestDescriptionCandidates: buildContextPreparationResult.manifestDescriptionCandidates,
            modules: buildContextPreparationResult.modules.map((module) => ({
              id: module.id,
              name: module.name,
              root: module.root
            }))
